...
data-tooltip="${title}" original-title="${title}"><img src="${url}"></span>
    ...
    async function filterIssues(status = null, searchTerm = ""){
        ...
        avatar = avatar.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0];
        ...
        for( let issue of issuesArr){
            ...
            const name = (issue.querySelector(".assignee")).childNodes[0].childNodes[1].textContent;
            ...
            if(!(summary+key+name).trim().toLowerCase().includes(currentSearchTerm.trim().toLowerCase())){
                issue.classList.add("display-none");
                if(!removedOne){
                    filtered++;
                    removedOne = true;
        ...
        for(let el of countEls){
            ...
            (el.querySelector(".results-count-total.results-count-link")).innerHTML = issuesArr.length;
        }
    ...
    async function addNewHeader(){
        ...
        let newHeader = createElementFromHTML(`<div class="release-report-tab-header" id="custom-header"></div>`);
        ...
        await statusNames.forEach( n => {
            ...
            const count = (Array.from(statuses).filter( x => x.textContent === n)).length;
...
<div id="not-me-box"><input id="not-me" type="checkbox"><label for="not-me">Exclude my tasks</label></div>`);
        ...
        if(!tabs.length){currentFilter = null};
        ...
        const avatarEls = await waitFor(".author-avatar-wrapper.avatar-with-name", 2000);

        const avatars = [...(new Set((Array.from(avatarEls)).map( x => {
        ...
        const imgContainer = createElementFromHTML(`<div class="custom-img-container"></div>`);
